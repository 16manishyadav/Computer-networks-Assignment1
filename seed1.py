    ...
    def handle_peer_connection(self, client_socket, addr):
        ...
        try:
            if message.startswith("REGISTER"):
                ...
                # Add peer port and ip to the peers file
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python script_name.py <seed_ip> <seed_port>")
        sys.exit(1)
    ...
    with open("config.txt", "a") as seeds_file:
        seeds_file.write(f"{seed_ip}:{seed_port}\n")

    seed = SeedNode(seed_ip, seed_port)
